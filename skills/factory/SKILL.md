...
3. Read `docs/agents/issue-tracker.md` — the Project's **Tracker adapter**. It
   names which tracker this Project uses, where its issues live, and the exact
   tooling every loop operation goes through. Never assume a tracker this file
   doesn't name. If it calls for tools whose schemas are deferred (an MCP
   server's, say), load them via ToolSearch before Session start.
...
- Mirror every transition in the tracker: pickup comment with branch name,
  completion comment with PR link, unstarted → started → completed. The
  adapter says what each of those states is for this Project's tracker.
