---
name: factory-new
description: Bootstrap a brand-new Factory Project end-to-end — repo under ~/apps/<name>, private GitHub remote, Linear project, full Factory stamp, triage labels — then hand off into a Planning Session. Use when the user runs /factory-new <name> to start a project from scratch.
---
...
team labels. It creates zero Linear issues. It ends by handing the maintainer
...
The skill makes no changes until all of them pass. This is what makes
re-running `/factory-new` on a name that already exists safe: it aborts
before creating a second anything.
...
4. No Linear project with the derived name (see below) may already exist on
   the Side projects team. Check with `list_projects` (filter to team "Side
   projects") before creating anything. Stop if found.
5. `gh auth status` must report a clean, authenticated state. Stop otherwise.
...
answer as `{{PROJECT_NAME}}` for the rest of this skill.
...
4. **Apply the stamp**: for each row in `templates/README.md`'s file mapping,
   ...
   not part of the stamp — do not create them here.
...
6. **GitHub remote**: `gh repo create <name> --private --source=. --remote=origin --push`.
   ...
   matching how `gh` is authenticated — not an `https://` URL.
...
## End state: hand off to a Planning Session
