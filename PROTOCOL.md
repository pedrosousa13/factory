...
- **When**: only for a decision the maintainer alone can make — a genuine fork
  ...
  have looked up.
- **Ping**: send a push notification with the question in one line, then wait
  ~15 minutes *without ending the turn* — a background wait (e.g. a background
  `sleep`), so a mid-turn answer from the maintainer arrives and cancels the
  wait. Ending the turn to ask is not blocking; it stalls the loop
  indefinitely, the exact cost Park exists to remove (ADR-0001).
- **Answered in time** → cancel the wait, continue the issue with context
  intact. No Park, no state change.
- **Unanswered** → Park, which is these four steps in order:
  ...
  2. **Post the question** as a Linear comment on the issue, opening with the
     AI disclaimer required of anything an agent posts to the tracker:
  ...
  4. **Continue** with the next Queue issue.
