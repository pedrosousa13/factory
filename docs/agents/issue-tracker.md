...
Issues and PRDs for this repo live as GitHub issues on **pedrosousa13/factory**.

Use the `gh` CLI for all operations. Pass `-R pedrosousa13/factory` explicitly on every
...
- **Create an issue**: `gh issue create -R pedrosousa13/factory --title "..." --body
  "..."`. Title in imperative mood; use a heredoc for multi-line bodies.
- **Read an issue**: `gh issue view <n> -R pedrosousa13/factory --json
...
- **List issues**: `gh issue list -R pedrosousa13/factory --state open --limit 500 --json
...
- **Comment**: `gh issue comment <n> -R pedrosousa13/factory --body "..."`.
- **Apply / remove labels**: `gh issue edit <n> -R pedrosousa13/factory --add-label
  "..."` / `--remove-label "..."`.
- **Close**: `gh issue close <n> -R pedrosousa13/factory --reason completed` (resolved)
  or `--reason "not planned"` (wontfix).

## When a skill says "publish to the issue tracker"

Create a GitHub issue on pedrosousa13/factory.

## When a skill says "fetch the relevant ticket"

Run `gh issue view <n> -R pedrosousa13/factory --json
...
- **Reachability**: `gh auth status` resolves the `gh` CLI and confirms it
  is authenticated; `gh repo view pedrosousa13/factory --json name` confirms the
  **pedrosousa13/factory** repo exists and is visible to this account.
- **Queue listing**: `gh issue list -R pedrosousa13/factory --state open --label
...
- **State: started**: `gh issue edit <n> -R pedrosousa13/factory --add-label
  ...
  the started state.
- **State: completed / canceled**: `gh issue close <n> -R pedrosousa13/factory --reason
  ...
  has no label flag: `gh issue edit <n> -R pedrosousa13/factory --add-label wontfix`,
  then `gh issue close <n> -R pedrosousa13/factory --reason "not planned"`, which reads
...
- **Park**: `gh issue edit <n> -R pedrosousa13/factory --remove-label ready-for-agent
...
- **Blocking**: GitHub's native **sub-issues**. Link one with `gh issue
  edit <parent> -R pedrosousa13/factory --add-sub-issue <child>`; read them back with
  `gh issue view <n> -R pedrosousa13/factory --json subIssues,subIssuesSummary`:
...
- **Milestone**: a GitHub **milestone** on the issue, not a label. Create
  one with `gh api repos/pedrosousa13/factory/milestones -f title=... -f
  description=...`; list a repo's milestones with `gh api --paginate
  "repos/pedrosousa13/factory/milestones?state=all&per_page=100"`, which returns them
  ...
  <n-or-title>` at creation, or `gh issue edit <n> -R pedrosousa13/factory --milestone
  <n-or-title>` afterwards. Read a milestone's completion with `gh api
  repos/pedrosousa13/factory/milestones/<n>` and its `open_issues` / `closed_issues`
  counts — GitHub reports no percentage, so compute one from the pair.
- **Milestone issue counts**: `gh issue list -R pedrosousa13/factory --state all
  ...
  **unstarted**. `--limit` is not optional here either: leave it off and
  ...
  `ready-for-agent`.
- **Open issues**: `gh issue list -R pedrosousa13/factory --state open --milestone
...
- **Read an issue**: `gh issue view <n> -R pedrosousa13/factory --json
  ...
  together. `gh issue view <n> -R pedrosousa13/factory --comments` renders the same
  discussion for a human to read, but a session working from the issue
  needs only the `--json` call.
- **Comment**: `gh issue comment <n> -R pedrosousa13/factory --body "..."`. Body as
...
- **State verification**: `gh issue view <n> -R pedrosousa13/factory --json
...
- **The map**: an ordinary issue on **pedrosousa13/factory** labeled `wayfinder:map`.
  Find a Project's maps with `gh issue list -R pedrosousa13/factory --state open
  --label wayfinder:map --limit 500 --json number,title`.
- **Labels**: `wayfinder:map`, `wayfinder:research`, `wayfinder:prototype`,
  `wayfinder:grilling`, `wayfinder:task` — repo labels on **pedrosousa13/factory**,
  created lazily by the first charting session: `gh label list -R pedrosousa13/factory`
  first, then `gh label create <label> -R pedrosousa13/factory` only for the names that
...
- **Child tickets**: GitHub's native sub-issues — `gh issue edit <map> -R
  pedrosousa13/factory --add-sub-issue <ticket>`, the same relation the loop's
...
- **Frontier**: read the map's children from `gh issue view <map> -R
  pedrosousa13/factory --json subIssues`, keep the `OPEN` ones, then confirm each
  candidate with its own `gh issue view <n> -R pedrosousa13/factory --json
  ...
  listing lags.
- **Claim**: `gh issue edit <n> -R pedrosousa13/factory --add-assignee @me` — the
...
- **Resolve**: post the resolution with `gh issue comment`, then `gh issue
  close <n> -R pedrosousa13/factory --reason completed`. A ticket ruled out of scope
...
and the **pedrosousa13/factory** repo exists and is visible — `gh auth status`, then
`gh repo view pedrosousa13/factory --json name`.
